...
Report the observed used/remaining percent, reset time, projected hit time, generated SVG/JSON paths, and any warnings. If the tool says `unknown`, keep it unknown. Call it an estimate from local logs because sparse logging, bursty usage, plan changes, and server enforcement can differ.
